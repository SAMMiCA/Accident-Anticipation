...
def evaluation_accident(all_pred, all_labels, time_of_accidents, fps=30.0):
    ...
    for idx, toa in enumerate(time_of_accidents):
        if all_labels[idx] > 0:
            foa = int(toa * fps)        # frame of accident
    ...
    thresolds[0] += 1e-6
    thresolds[-1] -= 1e-6
    # Precision Recall averageTimeToAccident
    ...
    # when trheshold=0.8
    p08, r08, t08 = compute_metrics(preds_eval, all_labels, frame_of_accidents, [0.5])
    ...
    return AP, mTTA, TTA_R80, p05[0], r05[0], t05[0] , p08[0], r08[0], t08[0] / fps
