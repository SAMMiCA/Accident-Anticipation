...
class QNetwork(nn.Module):
    ...
    def forward(self, state, action, detach=False):
        if self.arch_type == 'rae':
            if len(self.state_encoder(state, detach=detach).shape) == 3:
                xu = torch.cat([self.state_encoder(state, detach=detach), action], 2)
            else:
...
class FixationPolicy(nn.Module):
    ...
    def forward(self, state, detach=False):
        ...
        if self.policy_type == 'Gaussian':
            # log_std = self.log_std_linear(x)
            # log_std = torch.clamp(log_std, min=LOG_SIG_MIN, max=LOG_SIG_MAX)
            log_std = torch.tensor(0.0).to(mean.device)             #### to make deterministic
        else:
