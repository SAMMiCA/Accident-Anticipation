...
class ReplayMemory:
    ...
    def push(self, state, action, reward, next_state, rnn_state1, rnn_state2, rnn_state3, labels, done):
        state_cpu = state.cpu().numpy().flatten()
        action_cpu = action.flatten()
        next_state_cpu = next_state.cpu().numpy().flatten()
        rnn_state_cpu1 = torch.cat((rnn_state1[0].unsqueeze(0), rnn_state1[1].unsqueeze(0)), dim=0).cpu().numpy().reshape(-1, rnn_state1[0].size(-1))
        rnn_state_cpu2 = torch.cat((rnn_state2[0].unsqueeze(0), rnn_state2[1].unsqueeze(0)), dim=0).cpu().numpy().reshape(-1, rnn_state2[0].size(-1))
        rnn_state_cpu3 = torch.cat((rnn_state3[0].unsqueeze(0), rnn_state3[1].unsqueeze(0)), dim=0).cpu().numpy().reshape(-1, rnn_state3[0].size(-1))

        if len(self.buffer) < self.capacity:
            self.buffer.append(None)
        self.buffer[self.position] = (state_cpu, action_cpu, reward, next_state_cpu, rnn_state_cpu1, rnn_state_cpu2, rnn_state_cpu3, labels, done)
        self.position = (self.position + 1) % self.capacity

    def sample(self, batch_size, device):
        batch = random.sample(self.buffer, batch_size)
        state, action, reward, next_state, rnn_state1, rnn_state2, rnn_state3, labels, mask = map(np.stack, zip(*batch))
        # move data to GPU
        if rnn_state1[:, 0] is not None:
            rnn_state1 = (torch.from_numpy(rnn_state1[:, 0]).to(device), torch.from_numpy(rnn_state1[:, 1]).to(device))
        if rnn_state2[:, 0] is not None:
            rnn_state2 = (torch.from_numpy(rnn_state2[:, 0]).to(device), torch.from_numpy(rnn_state2[:, 1]).to(device))
        if rnn_state3[:, 0] is not None:
            rnn_state3 = (torch.from_numpy(rnn_state3[:, 0]).to(device), torch.from_numpy(rnn_state3[:, 1]).to(device))
        ...
        return state, action, reward, next_state, rnn_state1, rnn_state2, rnn_state3, labels, mask
...
class ReplayMemoryGPU:
    def __init__(self, cfg, batchsize, gpu_id, device):
        ...
        self.dim_mem = self.dim_state + self.dim_action + 1 + self.dim_state + 6 * self.dim_hidden + self.dim_labels + 1
    ...
    def push(self, state, actions, reward, next_state, rnn_state1, rnn_state2, rnn_state3, labels, done):
        ...
        """
        rnn_state_gpu1 = torch.cat(rnn_state1, dim=-1)
        rnn_state_gpu2 = torch.cat(rnn_state2, dim=-1)
        rnn_state_gpu3 = torch.cat(rnn_state3, dim=-1)
        transition = torch.cat((state, actions, reward, next_state, rnn_state_gpu1, rnn_state_gpu2, rnn_state_gpu3, labels, done), dim=-1)  # (B, 444)
    ...
    def sample(self, batch_size, device):
        ...
        rnn_state1 = (data_batch[:, :, start: start + self.dim_hidden].view(-1, self.dim_hidden), 
                     data_batch[:, :, start + self.dim_hidden: end].view(-1, self.dim_hidden))
        start, end = start + self.dim_state, end + 2 * self.dim_hidden
        rnn_state2 = (data_batch[:, :, start: start + self.dim_hidden].view(-1, self.dim_hidden), 
                     data_batch[:, :, start + self.dim_hidden: end].view(-1, self.dim_hidden))
        start, end = start + self.dim_state, end + 2 * self.dim_hidden
        rnn_state3 = (data_batch[:, :, start: start + self.dim_hidden].view(-1, self.dim_hidden), 
                     data_batch[:, :, start + self.dim_hidden: end].view(-1, self.dim_hidden))
        # labels
